"use strict";
let lzmaDecoderLoadPromise = null;
...
const dataUtils = {
    ...
    ensureLzmaDecoderLoaded() {
        ...
        if (!lzmaDecoderLoadPromise) {
            lzmaDecoderLoadPromise = new Promise((resolve, reject) => {
                ...
                script.onload = () => typeof LZMA !== "undefined"
                    ? resolve()
    ...
    async lzmaDecompress(compressed) {
        ...
        return new Promise((resolve, reject) => {
            LZMA.decompress(Array.from(compressed), (result, error) => {
                if (error)
                    reject(new Error("LZMA decoding failed: " + error));
                else {
