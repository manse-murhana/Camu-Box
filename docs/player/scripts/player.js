...
const { logBox, logSummary, playBtn, stopBtn, playbackControls, trackInfo, trackTitle, trackDetails, nfcWarning, } = elements;
...
async function loadMidiFromText(text) {
    ...
    try {
        ...
        if (isPlaying || playingNoteSequence)
            stopAudioPlayback();
        ...
        log(`Number of tracks: ${midiParsed.tracks.length}, length: ${midiParsed.duration.toFixed(2)}s`);
        ...
        const midiTitle = playerUtils.detectAndDecodeText(midiParsed.header?.name) || "MIDI Ready";
...
function setPlayState(playing) {
    ...
    if (nfcWarning)
        nfcWarning.style.display = midiReady ? "none" : "block";
}
function updatePlayButtonState() {
    const wasDisabled = playBtn.disabled;
    setPlayState(isPlaying);
    if (wasDisabled && !playBtn.disabled)
        log("Ready to play!");
}
function convertToNoteSequence(parsedMidi) {
    const notes = parsedMidi.tracks.flatMap((track) => {
...
async function playWithAudio() {
    if (!midiParsed)
        return log("MIDI data is not loaded yet");
